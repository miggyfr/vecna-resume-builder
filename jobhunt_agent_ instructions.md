...
- If I ask you to "study this job posting" from a specific company (and provide the job posting details), 
  ...
  Then "Analyze the job posting details" against my the attached files (e.g., @BaseResume, @WorkIdidatHawkinsLab.md and @WorkIdidatTheUpsideDownSyndicate.md, etc) to determine if I am a good fit. During this analysis, evaluate each job requirement by comparing it directly to your experience in the attached files. Identify only the requirements that are a strong or partial match to your skills, experience, or background—do not consider or include any requirements that have no match. For each matching requirement, note which of your skills is the strongest fit and how it benefits the job.  
  Then provide a reasoned assessment, highlighting matches in skills, experience, and requirements, as well as any gaps.
  Then "Make a summary report" following the steps in @prompts/Review_Actions_in_FULLAUDITMODE.md. In this report, include a clear explanation of which of your skills is strongest compared to each job requirement, based only on matches from your attached files. Do not include or reference any job requirements that do not match your experience.  
  Then "Make a job app pack"  
...
- If I ask you to "make a job app pack" for a specific company (e.g., "Make a job app pack"), follow these steps:
  ...
  3. For each action task, generate a tailored answer based on @WorkIdidatHawkinsLab.md and @WorkIdidatTheUpsideDownSyndicate.md and @keyskills_template_CreelHenry.md. Only use details from these files—do not invent or add any skills, experiences, or achievements that are not present in them, even if they might seem to fit a job requirement.  
